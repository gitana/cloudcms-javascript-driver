import * as CloudCMS from "../..";
var assert = require('chai').assert;

describe('node22', function() {
    it('should run node test without error using object bound functions', function(done) {

        (async function() {

            var session = await CloudCMS.connect();

            var repository = await session.createRepository();
            var branch = await repository.readBranch("master");

            // build out hierarchy
            var node = await branch.createNode({ "title": "node1"});

            // add a feature
            await node.addNodeFeature("f:filename", { "filename": "woo" });

            // read the node back
            setTimeout(async function() {
                console.log("m1");
                var node2 = await branch.readNode(node._doc);

                // verify it has the right "f:filename"
                var filename = node2._features["f:filename"].filename;
                assert.equal("woo", filename);
                console.log("m2: " + filename);

                done();
            }, 3000);

        })();
    });
});